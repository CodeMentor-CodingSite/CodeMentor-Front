...
function EditUserProfile({data}:{data:{userId:string; email:string; imgSrc:string | null}}) {
    return (
        <>
            <section
                ...
                <div className='w-[285px] flex flex-col space-y-[10px] items-center'>
                    <UserAvatar imageSrc={data.imgSrc}/>
                    <EditProfileImageButton/>
                </div>
                <div className='flex items-center space-x-2 min-w-[160px] px-[20px]'>
                    <span className='text-[24px] text-gray-600 dark:text-white font-medium'>{data.userId}</span>
                    <Link href='/userBoard'>
                        <IconBackToUserBoard/>
                    </Link>
                </div>
            ...
            <section className='-translate-y-[80px] flex flex-col space-y-6 max-w-[855px] mx-auto px-[32px] pt-[32px] pb-[52px]
             shadow-[0_0px_8px_1px_rgba(0,0,0,.18)] rounded-[8px] bg-white dark:bg-[#121212]'>
                ...
                <InfoEditor label='ID' value={data.userId} inputType='text'/>
                <InfoEditor label='Email' value={data.email} inputType='email'/>
